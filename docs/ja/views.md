...
| Agents                     | タスクに関連する agent (`✽` ClaudeCode / `⬢` Codex / `⛑` CopilotCli)。現在設定されている agent に加え、過去のセッションマーカーが残っている agent もアイコン表示される (`✽ ⬢ ⛑` のように複数並ぶ)。実行中はアクティブ agent がブランドカラー、それ以外はグレー |
...
| `g`       | 先頭にジャンプ                                                |
| `G`       | 末尾にジャンプ                                                |
| `Enter`   | Summary 行なら該当 file の diff へジャンプ (他の行では no-op) |
| `R`       | Diff を再取得 (カーソル位置・コメントを保持)                  |
| `O`       | Split ↔ fullscreen 切り替え                                   |
| `q`       | Diff view を閉じる (再度開くと状態を復元)                     |
