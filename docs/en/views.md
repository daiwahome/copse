...
| Agents                 | Agents associated with the task (`✽` ClaudeCode / `⬢` Codex / `⛑` CopilotCli). Shows the currently configured agent plus any agent that has a leftover session marker (multiple icons appear as `✽ ⬢ ⛑`). The active agent is shown in its brand color while running; grey otherwise |
